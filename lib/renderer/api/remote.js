  ...
  return function remoteMemberFunction (...args) {
    ...
    if (!isConstructCall && member.promisified) {
      return new Promise((resolve, reject) => {
        ipcRenderer.send(command, contextId, metaId, member.name, wrapArgs([...args, (error, result) => {
          if (error) {
            reject(error)
          } else {
            resolve(result)
    ...
    const result = ipcRenderer.sendSync(command, contextId, metaId, member.name, wrapArgs(args))
...
function makeRemoteFunction (meta) {
  return function remoteFunction (...args) {
    ...
    if (!isConstructCall && meta.promisified) {
      return new Promise((resolve, reject) => {
        ipcRenderer.send(command, contextId, meta.id, wrapArgs([...args, (error, result) => {
          if (error) {
            reject(error)
          } else {
            resolve(result)
    ...
    const result = ipcRenderer.sendSync(command, contextId, meta.id, wrapArgs(args))
    return metaToValue(result)
  }
}
